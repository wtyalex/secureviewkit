package com.wty.secureviewkit.widgets;

import com.wty.secureviewkit.widgets.utils.ColorUtils;
...
public class SwitchButton extends CompoundButton {
    ...
    private static final int[] CHECKED_PRESSED_STATE =
        new int[] {android.R.attr.state_checked, android.R.attr.state_enabled, android.R.attr.state_pressed};
    ...
    private static int getThemeAccentColorOrDefault(Context context,
        @SuppressWarnings("SameParameterValue") int defaultColor) {
        int colorAttr;
     ...
     * (moveRange - thumbWidth + max(thumbMargin.left, thumbMargin.right) + textThumbInset) backWidth = thumbRange +
     * thumbMargin.left + thumbMargin.right + max(textExtraSpace, 0) contentSize = thumbRange + max(thumbMargin.left, 0)
    ...
    private int measureWidth(int widthMeasureSpec) {
        ...
        if (widthMode == MeasureSpec.EXACTLY) {
            ...
            if (mThumbWidth != 0) {
                ...
                textExtraSpace = textWidth + mTextExtra
                    - (moveRange - mThumbWidth + ceil(Math.max(mThumbMargin.left, mThumbMargin.right)));
                mBackWidth = ceil(moveRange + mThumbMargin.left + mThumbMargin.right + Math.max(textExtraSpace, 0));
                if (mBackWidth < 0) {
                    mThumbWidth = 0;
                }
                if (moveRange + Math.max(mThumbMargin.left, 0) + Math.max(mThumbMargin.right, 0)
            ...
            moveRange = ceil(mThumbWidth * mThumbRangeRatio);
            textExtraSpace = ceil(textWidth + mTextExtra
                - (moveRange - mThumbWidth + Math.max(mThumbMargin.left, mThumbMargin.right) + mTextThumbInset));
            ...
            contentSize = ceil(moveRange + Math.max(0, mThumbMargin.left) + Math.max(0, mThumbMargin.right)
    ...
    private int measureHeight(int heightMeasureSpec) {
        ...
        if (heightMode == MeasureSpec.EXACTLY) {
            if (mThumbHeight != 0) {
                ...
                if (mBackHeight + getPaddingTop() + getPaddingBottom() - Math.min(0, mThumbMargin.top)
                    - Math.min(0, mThumbMargin.bottom) > heightSize) {
            ...
            if (mThumbHeight == 0) {
                mBackHeight = ceil(heightSize - getPaddingTop() - getPaddingBottom() + Math.min(0, mThumbMargin.top)
    ...
    private void setup() {
        ...
        mBackRectF.set(backLeft, mThumbRectF.top - mThumbMargin.top, backLeft + mBackWidth,
        ...
        if (mBackDrawable != null) {
            mBackDrawable.setBounds((int)mBackRectF.left, (int)mBackRectF.top, ceil(mBackRectF.right),
                ceil(mBackRectF.bottom));
        }

        if (mOnLayout != null) {
            float onLeft = mBackRectF.left
                + (mBackRectF.width() + mTextThumbInset - mThumbWidth - mThumbMargin.right - mOnLayout.getWidth()) / 2f
                - mTextAdjust;
        ...
        if (mOffLayout != null) {
            float offLeft = mBackRectF.right
                - (mBackRectF.width() + mTextThumbInset - mThumbWidth - mThumbMargin.left - mOffLayout.getWidth()) / 2f
                - mOffLayout.getWidth() + mTextAdjust;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (switchText != null && textRectF != null) {
            int alpha = (int)(255
                * (getProgress() >= 0.75 ? getProgress() * 4 - 3 : (getProgress() < 0.25 ? 1 - getProgress() * 4 : 0)));
        ...
        if (mIsThumbUseDrawable) {
            mThumbDrawable.setBounds((int)mPresentThumbRectF.left, (int)mPresentThumbRectF.top,
                ceil(mPresentThumbRectF.right), ceil(mPresentThumbRectF.bottom));
